/**
 * Modules in this bundle
 * 
 * kindlized:
 *   license: ISC
 *   author: https://twitter.com/iamchawan
 * 
 */
(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
var account, app;

app = require('./lib/app');

account = require('./lib/account');

angular.module('App', []).controller('postNewBook', ['$scope', '$filter', '$http', app]).controller('account', ['$scope', '$http', account]);



},{"./lib/account":2,"./lib/app":3}],2:[function(require,module,exports){
module.exports = function($scope, $http, account) {
  $scope.isSending = false;
  $scope.statusMessage = '送信中...';
  $scope.isRegist = !true;
  $scope.toggleBtn = function() {
    return $scope.isRegist = !$scope.isRegist;
  };
  $scope.regist = function(mail, password) {
    var httpOpt;
    $scope.isSending = true;
    httpOpt = {
      method: 'post',
      url: '/account/regist',
      data: {
        mail: mail,
        password: password
      }
    };
    $http(httpOpt).success(function(statusMessage) {
      $scope.statusMessage = statusMessage;
    }).then(function() {
      console.log('/account/regist 完了');
    });
  };
  $scope.login = function(mail, password) {
    var httpOpt;
    console.log(mail);
    console.log(password);
    $scope.isSending = true;
    httpOpt = {
      method: 'post',
      url: '/account/login',
      data: {
        mail: mail,
        password: password
      }
    };
    $http(httpOpt).success(function(statusMessage) {}).then(function() {
      console.log('/account/regist 完了');
    });
  };
};



},{}],3:[function(require,module,exports){
module.exports = function($scope, $filter, $http) {
  var httpOpt;
  $scope.isRegiterd = false;
  httpOpt = {
    method: 'get',
    url: '/book/user'
  };
  $http(httpOpt).success(function(bookListInUser, status, header, config) {
    $scope.bookListInUser = bookListInUser;
  }).then(function() {
    console.log('/book/user 完了');
  });
  $scope.search = function(newBook) {
    var httpOptToAmazon, httpOptToDB;
    console.log(newBook, 'postAuthor clicked');
    $scope.isWaiting = true;
    httpOptToDB = {
      method: 'post',
      url: '/search/db',
      data: {
        newBook: $scope.newBook
      }
    };
    httpOptToAmazon = {
      method: 'post',
      url: '/search/amazon',
      data: {
        newBook: $scope.newBook
      }
    };
    $http(httpOptToDB).success(function(data, status) {
      $scope.bookListInDB = data.bookListInDB;
    }).then(function() {
      console.log('/search/db 完了');
    });
    $http(httpOptToAmazon).success(function(data, status) {
      $scope.bookListInAmazon = data.bookListInAmazon;
    }).then(function() {
      console.log('/search/amazon 完了');
      $scope.isWaiting = false;
      $scope.newBook = '';
    });
  };
  $scope.registBook = function(newBook, $index) {
    console.log(newBook);
    httpOpt = {
      method: 'post',
      url: '/save',
      data: {
        newBook: newBook
      }
    };
    $http(httpOpt).success(function(data, status) {
      console.log('registBook satatus is', status);
      $scope.bookListInDB[$index].isRegisterd = true;
      $scope.isRegiterd = true;
      $scope.bookListInUser.push($scope.bookListInDB[$index]);
    }).then(function() {
      console.log('/save 完了');
    });
  };
  $scope.unRegistBook = function($index) {
    var _id;
    _id = $scope.bookListInUser[$index]._id;
    $scope.bookListInUser.splice($index, 1);
    httpOpt = {
      method: 'post',
      url: '/reduce',
      data: {
        deleteBookId: _id
      }
    };
    $http(httpOpt).success(function() {}).then(function() {
      console.log('/reduce 完了');
    });
  };
};



},{}]},{},[1]);
